...
# function which performs the required processing of the image
def process_img(imge):
    '''
    process_img(imge) function is used to perform all the required image processing for lane detection

    input: imge - numpy.ndarray, the image that needs to be processed
    output: edg - numpy.ndarray, the processed image
    '''
    # removing noise


    #img_eq = cv2.fastNlMeansDenoising(imge, None, 10, 7, 21,)
    # adaptive thresholding

    #img_eq = cv2.adaptiveThreshold(img_eq, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 7, 2,dtype='uint8')
    # removing any noise which was amplified due to thresholding
    #img_eq = cv2.fastNlMeansDenoising(img_eq, None, 10, 7, 21)
    # edge detection
    edg = cv2.Canny(imge, 2, 50)
    # removing any noise which was amplified by the high pass filter
    ...
    if left_or_right in ['left', 'Left', 'LEFT']:
        for x1, y1, x2, y2, rho, theta in lines:
            ...
            if theta > 0 and theta < np.pi / 2:  # the angle of left lane is assumed to be between 0and 90 degrees
                score = (-1 * angle_weight * theta) + (-1 * dist_weight * (
                            right_margin - intercept))  # lower the distance from center of the two lanes, higher the score, steeper the line higher the score
    ...
    if left_or_right in ['right', 'Right', 'RIGHT']:
        for x1, y1, x2, y2, rho, theta in lines:
            ...
            if theta < np.pi and theta > np.pi / 2:  # the angle of right lane is assumed to be between 180 and 90 degrees
                score = (angle_weight * theta) + (-1 * dist_weight * (
                            intercept - left_margin))  # lower the distance from center of two lanes, higher the score, steeper the line higher the score
...
def Kalman_Filter_Update(C, Ex, Ez, Q_est, P_est, z):
    ...
    if Q_est.size:
        ...
        K = dot_prod(dot_prod(P_est, C.transpose()), np.linalg.inv(dot_prod(C, dot_prod(P_est,
                                                                                        C.transpose())) + Ez))  # Kalman gain (The weightage that needs to be given for the discrepency in measurement and the prediction to update the state and its covariance )

        # Step 4: State_at_t = Est_of_state_at_t + Kalman_gain_at_t (measured_variable_at_t - (C * Est_of_state_at_t) )

        Q = Q_est + dot_prod(K, (z - dot_prod(C,
                                              Q_est)))  # correcting state estimate using measured variables to obtain actual state

        # Step 5: State_cov_at_t = (I - Kalman_gain_at_t * C) Est_of_state_cov_at_t

        P = dot_prod((np.identity(4) - dot_prod(K, C)),
                     P_est)  # correcting state covariance estimate using measured variables to obtain actual state covariance
...
def elimnate_predicted_background(lane, img, img_width, img_height, top_margin, left_margin, height, Q_est,
                                  prediction_error_tolerance):
    '''
    ...
    if (lane in ["left", "Left", "LEFT"]):
        for x in range(0, img_width):
            for y in range(0, img_height):
                if y < (y1 - top_margin - (
                        ((x + left_margin) - (x1 - prediction_error_tolerance)) * (y1 - y2) / (x2 - x1))) or y > (
                        y1 - top_margin - (
                        ((x + left_margin) - (x1 + prediction_error_tolerance)) * (y1 - y2) / (x2 - x1))):
                    img[y][x] = 0
...
if __name__ == "__main__":
    ...
    while (cap.isOpened()):
        ...
        predictions = []
        predictions.append(
            ("Left_lane_intercept_estimate", "Left_lane_intercept_actual", "Left_lane_intercept_measured",
             "Left_lane_angle_estimate", "Left_lane_angle_actual", "Left_lane_angle_measured",
             "Right_lane_intercept_estimate", "Right_lane_intercept_actual", "Right_lane_intercept_measured",
             "Right_lane_angle_estimate", "Right_lane_angle_actual", "Right_lane_angle_measured"))
        ...
        top_margin = int(img.shape[0] * 400 / 720)  # pixels
        bottom_margin = int(img.shape[0] * 680 / 720)  # pixels
        left_lane_left_margin = int(img.shape[1] * 50 / 1280)  # pixels
        left_lane_right_margin = int(img.shape[1] * 525 / 1280)  # pixels
        right_lane_left_margin = int(img.shape[1] * 525 / 1280)  # pixels
        right_lane_right_margin = int(img.shape[1] * 1000 / 1280)  # pixels
        height = img.shape[0]

        # load test image
        img_test = frame
        left_lane_img = img_test[top_margin:bottom_margin, left_lane_left_margin:left_lane_right_margin]
        right_lane_img = img_test[top_margin:bottom_margin, right_lane_left_margin:right_lane_right_margin]
        ...
        Ex = np.array([[(dt ** 4) / 4, 0, (dt ** 3) / 2, 0], [0, (dt ** 4) / 4, 0, (dt ** 3) / 2],
                       [(dt ** 3) / 2, 0, (dt ** 2), 0], [0, (dt ** 3) / 2, 0, (dt ** 2)]]) * (
                     acc_noise ** 2)  # State prediction error
        ...
        while ((not left_lane) or (not right_lane)):
            ...
            # image processing
            left_img_pr = process_img(left_lane_img)
            right_img_pr = process_img(right_lane_img)

            # Elimination of Background based on Kalman Filtering

            if left_Q_est.size and trial == 1:
                left_img_pr = elimnate_predicted_background("left", left_img_pr, left_lane_img.shape[1],
                                                            left_lane_img.shape[0], top_margin,
                                                            left_lane_left_margin,
                                                            height, left_Q_est, prediction_error_tolerance)

            if right_Q_est.size and trial == 1:
                right_img_pr = elimnate_predicted_background("right", right_img_pr, right_lane_img.shape[1],
                                                             right_lane_img.shape[0], top_margin,
                                                             right_lane_left_margin, height, right_Q_est,
                                                             prediction_error_tolerance)
            ...
            if (not left_lane):
                ...
                left_lane = decide_lanes("left", left_lines, img.shape[0], top_margin, left_lane_left_margin,
                                         left_lane_right_margin)

            if (not right_lane):
                right_lines = detect_best_lines(right_img_pr, 170, 90, 75, 105)
                right_lane = decide_lanes("right", right_lines, img.shape[0], top_margin, right_lane_left_margin,
                                          right_lane_right_margin)
            ...
            if left_lane:
                (left_Q, left_P) = Kalman_Filter_Update(C, Ex, Ez, left_Q_est, left_P_est,
                                                        np.array([[left_lane[-1]], [left_lane[4]]]))

            if right_lane:
                (right_Q, right_P) = Kalman_Filter_Update(C, Ex, Ez, right_Q_est, right_P_est,
                                                          np.array([[right_lane[-1]], [right_lane[4]]]))
            if (trial >= 3):
                if not left_lane:
                    x1 = left_Q_est[0][0]
                    y1 = height
        ...
        if len(left_lane) == 0:
            ...
            left_x = left_lane[-1]
            cv2.line(img, (int(left_lane[0]), int(left_lane[1])), (int(left_lane[2]), int(left_lane[3])), color)
        if len(right_lane) == 0:
            right_x = 'None'
        else:
            right_x = right_lane[-1]
            cv2.line(img, (int(right_lane[0]), int(right_lane[1])), (int(right_lane[2]), int(right_lane[3])), color)

        # Sample intercepts
        #array = np.arange(0, 921600, 1, np.uint8)
        #array = np.reshape(array, (1280, 720))
        #PIL_image = Image.fromarray(np.uint8(frame)).convert('RGB')

        #PIL_image = Image.fromarray(frame.astype('uint8'), 'RGB')


        intercepts.append((frame, left_x, right_x))
        if (left_Q_est.size and right_Q_est.size and left_Q.size and right_Q.size):
            predictions.append((left_Q_est[0][0], left_Q[0][0], left_lane[-1], left_Q_est[1][0], left_Q[1][0],
                                left_lane[4], right_Q_est[0][0], right_Q[0][0], right_lane[-1], right_Q_est[1][0],
                                right_Q[1][0], right_lane[4]))
...
#with open('Kalman_filter.csv', 'w') as f:
#    writer = csv.writer(f)
